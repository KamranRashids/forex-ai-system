...
| Layer | Mechanism | Status (v0.1.0, verified 2026-09-12) |
|---|---|---|
...
| **L2 — Code** | Only a `PaperBroker` implements the `BrokerAdapter` interface (`app/broker/`); no live-broker module exists to enable. | ✅ active |
| **L3 — Orchestrator guard** | Decision pipeline hard-checks the mode before publishing any trade intent. | ✅ active |
...
| **L5 — Tests / CI** | SAFE MODE regression suite: config-layer rejection of every non-safe value runs in every PR (`tests/unit/test_config_safe_mode.py`, marked `safety`); pipeline-level assertions that zero non-paper orders can be emitted. | ✅ active |
...
Frontend dashboards display an amber “SAFE MODE — Paper only” badge driven by the
server-reported mode from `/system/status` (`isSafeMode`).
